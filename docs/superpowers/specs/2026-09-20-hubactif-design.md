...
- **Cycle de vie des données** : le hub ne contient que des codes (aucun nom). Une **remise à zéro annuelle est proposée à partir du 15 juillet** (bandeau dans l'espace enseignant, déclenchée par l'enseignant, une action qui supprime élèves, cibles, événements, liens et notes de la classe). **Purge automatique à partir du 15 août** pour les classes non remises à zéro : le cron tourne chaque jour d'août, la fonction refuse de s'exécuter avant le 15 août, ne touche jamais une classe créée après le 15 juillet de l'année scolaire en cours et est idempotente (migration `20260920130000_hub_purge_window.sql`). Retirer un élève est un retrait doux (`active = false`, historique conservé, réactivable) ; seule la remise à zéro supprime définitivement. À partir du 16 août, l'enseignant peut préparer sa nouvelle année (nouvelles classes, nouveaux codes). Pendant l'année, l'enseignant gère librement ses classes : ajout et suppression de classes, ajout et retrait d'élèves. Supprimer une classe supprime en cascade tout ce qui s'y rattache. De nouveaux codes sont générés chaque année. Les fiches locales des apps ne sont pas touchées par cette remise à zéro (elles relèvent de chaque app).
